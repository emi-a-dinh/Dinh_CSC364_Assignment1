...
def generate_forwarding_table_with_range(table):
    ...
    for row in table: 
        ...
        if destination != "0.0.0.0": 
            ...
            network_dst_string = destination
            ...
            # 7. Build the new row.
            # network destination, netmask, source, interface, ip range
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    while True:
        ...
        if sending_port == "8005": 
            ...
            write_to_file("output/sent_by_router_5.txt", str(new_packet), "5")
            router5.send(",".join(new_packet).encode('utf-8'))
        elif sending_port == "8006":
            print("sending packet", new_packet, "to Router 6")
            write_to_file("output/sent_by_router_6.txt", str(new_packet), "6")
